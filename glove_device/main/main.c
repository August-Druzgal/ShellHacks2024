...
static const char local_device_name[] = "August Glove Device";
...
static struct gpio_pin_info pin_info[NUM_GPIOS];
...
void gpio_task_fn(void *args)
{
    while (true)
    {
        for (int i = 0; i < NUM_GPIOS; i++)
        {
            ...
            if (pin_info[i].frequency == 0)
            {
                gpio_set_level(i, 0);
                continue;
            }

            if (pin_info[i].counter == 0)
            {
                if (pin_info[i].set)
                {
                    gpio_set_level(i, 0);
                    pin_info[i].set = false;
                    pin_info[i].counter = pin_info[i].frequency;
                }
                else
                {
                    gpio_set_level(i, 1);
                    pin_info[i].set = true;
                    pin_info[i].counter = 20;
                }

                // printf("Setting gpio %d to %s\n", i, pin_info[i].set ? "high" : "low");
            }

            pin_info[i].counter -= 1;
        }
...
esp_err_t gpio_init(void)
{
    for (int i = 0; i < NUM_GPIOS; i++)
    {
        pin_info[i].counter = 0;
        pin_info[i].frequency = 0;
        pin_info[i].set = false;
        pin_info[i].ready = false;
    }
...
{
    if (pin_num >= NUM_GPIOS || pin_num < 0)
        return ESP_ERR_INVALID_ARG;

    pin_info[pin_num].ready = true;
    ...
    return gpio_set_level(pin_num, 0);
}

esp_err_t gpio_start_task(void)
{
    xTaskCreate(gpio_task_fn, "gpio-task", 2048, NULL, 5, NULL);
...
{
    if (pin_level > 1000 || pin_level < 0)
        return ESP_ERR_INVALID_ARG;

    if (pin_num >= NUM_GPIOS || pin_num < 0)
        return ESP_ERR_INVALID_ARG;

    if (!pin_info[pin_num].ready)
        return ESP_ERR_INVALID_ARG;

    pin_info[pin_num].frequency = pin_level;
...
void gpio_task_start(void)
{
    ...
    {
        init_gpio_pin(i + 12);
        gpio_set_pin_frequency(i + 12, 0);
    }

    xTaskCreate(gpio_task_fn, "gpio_task", 2 * 1024, NULL, configMAX_PRIORITIES - 3, NULL);
}
...
void esp_spp_cb(esp_spp_cb_event_t event, esp_spp_cb_param_t *param)
{
    ...
    switch (event)
    {
        case ESP_SPP_DATA_IND_EVT:
            for (uint32_t i = 0; i < param->data_ind.len; i++)
                printf("%c", (char) param->data_ind.data[i]);
            printf("\n");

            if (param->data_ind.len >= 5)
                for (uint32_t i = 0; i < 5; i++)
                    gpio_set_pin_frequency(i + 12, param->data_ind.data[i]);
...
void app_main(void)
{
    ...
    esp_spp_start_srv(ESP_SPP_SEC_NONE, ESP_SPP_ROLE_SLAVE, 3, "ahhh");
